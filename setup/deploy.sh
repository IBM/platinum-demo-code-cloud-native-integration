...
#  sed "s#{{NAMESPACE}}#$namespace#g;" > $SCRIPT_DIR/operator-group.yaml

#oc apply -f $SCRIPT_DIR/operator-group.yaml

#rm $SCRIPT_DIR/operator-group.yaml

oc apply -f $SCRIPT_DIR/ibm-operator-catalog.yaml

cat $SCRIPT_DIR/mq-operator-subscription.yaml_template |
  sed "s#{{NAMESPACE}}#$namespace#g;" > $SCRIPT_DIR/mq-operator-subscription.yaml

oc apply -f $SCRIPT_DIR/mq-operator-subscription.yaml

rm $SCRIPT_DIR/mq-operator-subscription.yaml

wait_for_operator_start ibm-mq $namespace

cat $SCRIPT_DIR/ace-operator-subscription.yaml_template |
  sed "s#{{NAMESPACE}}#$namespace#g;" > $SCRIPT_DIR/ace-operator-subscription.yaml

oc apply -f $SCRIPT_DIR/ace-operator-subscription.yaml

rm $SCRIPT_DIR/ace-operator-subscription.yaml
